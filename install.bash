...
if [[ -f "$SCRIPT_DIR/cursor-agent-ask" ]] && git -C "$SCRIPT_DIR" rev-parse --is-inside-work-tree &>/dev/null; then
  DATA_DIR="$SCRIPT_DIR"
  ...
  echo "Cloning cursor-agent-ask..."
  git clone "$REPO_URL" "$DATA_DIR"
fi

mkdir -p "$BIN_DIR"
ln -sf "${DATA_DIR}/cursor-agent-ask" "$LINK"
echo "Installed: $LINK -> ${DATA_DIR}/cursor-agent-ask"
...
if ! python3 -c "import rich" 2>/dev/null; then
  echo ""
  echo "Python の 'rich' ライブラリが見つかりません。"
  echo "rich があると Markdown のリッチ表示（コードブロック、テーブル等）が有効になります。"
  echo "なくても動作しますが、プレーンテキスト表示になります。"
  echo ""
  read -rp "rich をインストールしますか？ [Y/n] " ans < /dev/tty
  if [[ "$ans" != [nN] ]]; then
    if command -v pip &>/dev/null; then
      ...
      echo "pip が見つかりません。手動でインストールしてください:"
      echo "  pip install rich"
    fi
  else
    echo "スキップしました。後からインストールするには: pip install rich"
  fi
fi
